...
新的在上面。只记录会影响使用的改动，措辞和排版的小修不单独列。

### 2026-09-10

- 定时任务常在电脑刚唤醒、网络还没连上时就跑起来，导致域名解析失败、抓取整站落空。
  现在开工前会先等域名可解析，最多等 5 分钟；网络正常时不产生任何延迟。
- 抓取重试从 3 次（共 6 秒）放宽到 5 次（共 30 秒）。
- 发信失败现在会重试。此前一次失败整封邮件就丢了，要等第二天才补发。

### 2026-09-09

- 新增 macOS 支持：`setup_launchd.sh` 一键注册 launchd 定时任务。错过的运行会在
  唤醒或开机后补跑，cron 没有这个行为。
- README 改为面向使用者重写，中英双语。
- 补上 `.gitignore` 与 `LICENSE`。`.gitignore` 尤其重要：它挡住 `config.json`，
  那里面有你的邮箱应用专用密码。

### 2026-08-23

- 首个版本：三个来源、每日邮件、Windows 计划任务、配置验证信。
